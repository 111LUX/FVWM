...
```
* dzen2 is an optional dependency, but with this package installed, you'll get nice notifications with current Desk number, when switching it with Super+F1/F2/F3/F4 or Super+PgUp/PgDown. Also, it may be used to display sound volume notifications, when changing it with your keyboard/laptop multimedia hotkeys (continue reading to know how).  
...
* stalonetray is used as a tray application. (Alternatively, it is possible to install wmsystemtray application instead, then uncomment two "wmsystemtray" lines at top of ~/.fvwm/config .)
...
* **Drop down terminal** is available, toggle its visibility with F1 hotkey. No additional actions required, it's working with every screen resolution "out of the box", urxvt terminal emulator will be used if installed, or otherwise xterm. It "remembers" its maximized state even when is hidden (maximize/unmaximize it with Super+W hotkey, or using tiling functionality - Super+Up/Down/Left/Right arrows).  
